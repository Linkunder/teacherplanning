...
class Evaluacion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('ponderacion, idCurso', 'numerical', 'integerOnly'=>true),
			array('nombre', 'length', 'max'=>45),
			array('descripcion, fecha', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idEvaluacion, nombre, descripcion, ponderacion, fecha, idCurso', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'alumnos' => array(self::MANY_MANY, 'Alumno', 'alumnorindeevaluacion(idEvaluacion, idAlumno)'),
			'Curso' => array(self::BELONGS_TO, 'Curso', 'idCurso'),
		);
	}
